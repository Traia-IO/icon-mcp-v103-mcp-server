...
# Script to build and run the ICON MCP v103 MCP Server locally in Docker
...
IMAGE_NAME="icon-mcp-v103-mcp-server"
CONTAINER_NAME="icon-mcp-v103-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running ICON MCP v103 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# ICON MCP v103 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
if ! grep -q "^ICON_MCP_V103_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your ICON MCP v103 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "ICON_MCP_V103_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^ICON_MCP_V103_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^ICON_MCP_V103_API_KEY=.*|ICON_MCP_V103_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^ICON_MCP_V103_API_KEY=.*|ICON_MCP_V103_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "ICON_MCP_V103_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set ICON_MCP_V103_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ ICON_MCP_V103_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ ICON MCP v103 MCP Server is running!${NC}"
echo
