# ICON MCP v103 MCP Server Implementation Guide

You are working on implementing the ICON MCP v103 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: ICON MCP v103
- **Documentation**: []()
- **Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **Authentication**: Required - API key via `ICON_MCP_V103_API_KEY` environment variable
...
When a client provides their own ICON MCP v103 API key:
...
Flow:
  1. Client connects with their ICON MCP v103 API key
  2. MCP server uses client's API key to call ICON MCP v103 API
  3. No payment required
  4. Client pays ICON MCP v103 directly (not the MCP server)
```

**Use Case**: Clients who already have a ICON MCP v103 subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL ICON MCP v103 API key to call the API
  5. Client pays the MCP server (not ICON MCP v103)
```

**Use Case**: Pay-per-use clients without their own ICON MCP v103 subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("ICON_MCP_V103_API_KEY")
...
**Required**:
- `ICON_MCP_V103_API_KEY`: Server's internal ICON MCP v103 API key (used when clients pay via 402)
...
ICON_MCP_V103_API_KEY=your_icon-mcp-v103_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_icon_mcp_v103",
      "get_icon_mcp_v103_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["icon mcp v103", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the ICON MCP v103 API with retry support. Example pattern:
...
def call_icon_mcp_v103_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call ICON MCP v103 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"ICON MCP v103 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_icon_mcp_v103(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search ICON MCP v103 for [specific data type].
    ...
    try:
        # The call_icon_mcp_v103_api function already has retry logic
        results = call_icon_mcp_v103_api(
...
```python
@mcp.tool()
async def get_icon_mcp_v103_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_icon_mcp_v103_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in ICON MCP v103.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_icon_mcp_v103(query="example", limit=5)
   ```
...
### 9. Common ICON MCP v103 Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on ICON MCP v103 capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the ICON MCP v103 documentation that you should implement:
...
- Check the ICON MCP v103 API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make ICON MCP v103's capabilities easily accessible to AI agents through standardized MCP tools. 